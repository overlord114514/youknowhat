...
# End of protobuf generated code
...
async def handle_message_request(request):
    global llm_token
    if not llm_token:
        logger.error("handle_message_request: LLM token not available")
        return web.json_response({"error": "LLM token not available"}, status=500)
    try:
        body = await request.json()
        logger.debug(f"handle_message_request: Received request body: {json.dumps(body, ensure_ascii=False)}")
        ...
        if "messages" in body:
            for msg in body["messages"]:
                ...
                if "role" in msg and msg["role"] == "system":
                    logger.warning("handle_message_request: Converting system role to user")
                    msg["role"] = "user"

        if "system" in body:
            if isinstance(body["system"], list):
                system_text = "\n".join([item["text"] for item in body["system"]])
                logger.debug(f"handle_message_request: Converting system field from list: {system_text}")
            else:
                system_text = body["system"]
            logger.warning(f"handle_message_request: Removing system field (content: {system_text}) as provider does not support it")
            del body["system"]

        if "max_tokens" not in body:
            body["max_tokens"] = 256
            logger.info("handle_message_request: Setting default max_tokens value to 256")
        ...
        logger.debug(f"handle_message_request: Forwarding payload to LLM API: {json.dumps(payload, ensure_ascii=False)}")
        ...
        logger.error(f"handle_message_request: Error processing request: {e}")
...
async def handle_non_streaming_request(headers, payload):
    async with aiohttp.ClientSession() as session:
        async with session.post(LLM_API_URL, headers=headers, json=payload) as r:
            logger.info(f"handle_non_streaming_request: LLM API returned status {r.status}")
            if r.status != 200:
                text = await r.text()
                logger.error(f"handle_non_streaming_request: LLM API error response: {text}")
            ...
            async for line in r.content:
                if not line:
                    continue
                logger.debug(f"handle_non_streaming_request: Received raw line: {line}")
                try:
                    event = json.loads(line.decode('utf-8').strip())
                    logger.debug(f"handle_non_streaming_request: Decoded event: {json.dumps(event, ensure_ascii=False)}")
                    et = event.get('type')
                    if et == "message_start":
                        pass
                        ...
                        logger.debug(f"handle_non_streaming_request: Appended delta: {delta_text}")
                    elif et == "message_delta" and 'usage' in event:
                        usage = event.get('usage')
                    elif et == "message_stop":
                        logger.debug("handle_non_streaming_request: Received message_stop event")
                        break
                except Exception as e:
                    logger.error(f"handle_non_streaming_request: Error processing line: {e}")
            ...
            logger.info(f"handle_non_streaming_request: Final response_data: {json.dumps(response_data, ensure_ascii=False)}")
...
async def handle_streaming_request(request, headers, payload):
    ...
    logger.info("handle_streaming_request: Starting streaming request to LLM API")
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(LLM_API_URL, headers=headers, json=payload, timeout=60) as api_response:
                logger.info(f"handle_streaming_request: LLM API returned status {api_response.status}")
                if api_response.status != 200:
                    error_text = await api_response.text()
                    logger.error(f"handle_streaming_request: LLM API error response: {error_text}")
                    try:
                        await response.write(f"data: {json.dumps({'error': error_text})}\n\n".encode())
                    except Exception as ex:
                        if "Cannot write to closing transport" in str(ex):
                            logger.warning("handle_streaming_request: Transport closing, skipping error message write")
                        else:
                            logger.warning(f"handle_streaming_request: Failed to write error message: {ex}")
                    try:
                        await response.write(b"data: [DONE]\n\n")
                    except Exception as ex:
                        if "Cannot write to closing transport" in str(ex):
                            logger.warning("handle_streaming_request: Transport closing, skipping final DONE write")
                        else:
                            logger.warning(f"handle_streaming_request: Failed to write DONE message: {ex}")
                    return response
                ...
                async for line in api_response.content:
                    ...
                    try:
                        ...
                        logger.error(f"handle_streaming_request: Failed to decode line: {decode_err}")
                        continue
                    ...
                    if decoded_line == "[DONE]":
                        try:
                            ...
                            if "Cannot write to closing transport" in str(e):
                                logger.warning("handle_streaming_request: Transport closing during [DONE] write")
                            else:
                                logger.warning(f"handle_streaming_request: Failed to write [DONE] message: {e}")
                        break

                    if decoded_line and isinstance(decoded_line, str):
                        try:
                            # 如有需要可在此包装 JSON 数据
                            wrapped = decoded_line
                            await response.write(f"data: {wrapped}\n\n".encode())
                        except Exception as e:
                            if "Cannot write to closing transport" in str(e):
                                logger.warning("handle_streaming_request: Transport closing, stopping further writes")
                                break
                            else:
                                logger.error(f"handle_streaming_request: Error writing SSE data: {e}")
                                break
                    else:
                        logger.debug("handle_streaming_request: Skipped empty or invalid line")
                
                try:
                    await response.write(b"data: [DONE]\n\n")
                except Exception as ex:
                    if "Cannot write to closing transport" in str(ex):
                        logger.warning("handle_streaming_request: Transport closing during final DONE write")
                    else:
                        logger.warning(f"handle_streaming_request: Failed to write final DONE message: {ex}")
    except Exception as outer_e:
        logger.error(f"handle_streaming_request: Outer error: {outer_e}")
    return response
